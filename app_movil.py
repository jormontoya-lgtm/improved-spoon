...
if not st.session_state.autenticado:
    ...
    if menu == "Reportar Avance":
        ...
        # Solo mostrar diámetros si es Tubería
        mat = "N/A"
        if act == "Tubería":
        ...
        conn.close()
        st.table(df) # Se ve perfecto en pantallas pequeñas

    elif menu == "Exportar":
        st.header("📊 Generar Reporte")
        if st.button("Descargar Excel"):
            st.info("En la versión web, esto genera el archivo para descarga inmediata.")